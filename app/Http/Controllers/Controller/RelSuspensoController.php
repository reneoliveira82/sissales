...
class RelSuspensoController extends Controller
{
    protected $pag = 'rel_suspenso';
    protected $model;
    ...
    public function tabelaAjax()
    {
        ...
        if ($this->checkPermission($this->view_permission))
            return redirect()->back();
        $result = $this->model::leftjoin('status', 'status.id', '=', 'servidors.id_status')->where('cod_status','=', 4)->get()->count();
        $registros = $this->model::leftjoin('status', 'status.id', '=', 'servidors.id_status')->select('servidors.*','status.cod_status', 'status.desc_status')->where('status.cod_status', 4)->get(); 
    ...
    public function buscaEmRecadastramento(Request $req){
        ...
        ->whereDate('servidors.updated_at','>=',$dt_inicial)->whereDate('servidors.updated_at','<=',$dt_final)->where('status.cod_status',4)->get();
        return view("view.$this->pag.table", compact('registros','result'));       
    }
}
